...
async function Mashup(thing, color) {
    ...
    cubeSortLoop();
    async function cubeSortLoop() {


        try {
            var obj = await servient.start();
            whitebotthing = await obj.consume(whiteBottd);
            infrasensorthing = await obj.consume(infrasensortd);
            thingConvBelt = await obj.consume(converyorbelt1td);
            thingUarm = await obj.consume(uarmtd);
            //move uarm to home
